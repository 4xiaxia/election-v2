...
## 2026-07-09 甲方后台最小可用 HTML 版

### 输入

- 夏夏纠偏：`换届选举系统-UI优化.html` 是子管理登录后台后看到的仪表盘，不是纸面说明稿。
- 夏夏要求保留旧稿，不删除；旧稿用于继续指出问题和供其他 agent 定位。

### 动作

- 新增目录 `deliverables/client-admin-html/`。
- 新增 `index.html`：
  - 默认打开就是 `经办 · 涧口社区` 首页工作台，不先出现登录页。
  - 左侧模块：首页工作台、选举申请、选举活动/母版、岗位管理、材料提交管理、候选人管理、公告通知/归档。
  - 首页展示当前角色、活动、岗位数、待审核材料、日历日程、本届选举入口。
  - 母版页展示阶段表，日期可编辑并保存到 localStorage。
  - 岗位页展示主任/副主任/委员，支持岗位详情弹窗。
  - 材料页支持筛选、审核通过、驳回；通过后进入候选人列表。
  - 候选人页支持结果回填。
  - 公告通知/归档页展示阶段公告和归档文件夹结构。
  - 通知弹窗支持标题、类型、时间、属性、手机号、文本内容。
- 新增 `README.md`：
  - 说明入口文件、定位、localStorage 保存和正式系统边界。
- 原 `换届选举系统-UI优化.html` 未删除，仍作为旧施工锚点和问题定位稿。

### 验证

- `index.html` 内脚本语法检查通过。
- Chrome headless 截图确认默认打开就是首页工作台，且日历日程已渲染。
- 关键文案可搜索：`首页工作台`、`选举活动 / 母版`、`材料提交管理`、`候选人管理`、`公告通知 / 归档`、`涧口社区`、`第十五届`、`localStorage`、`election_voters`、`不是线上投票`。
- `git diff --check -- deliverables/client-admin-html/index.html deliverables/client-admin-html/README.md` 通过。

### 接力棒

- 给甲方或夏夏看新目录 `deliverables/client-admin-html/index.html`。
- 旧 HTML 不删，用于继续对照夏夏原始要求和定位缺口。
- 下一刀如果继续打磨 UI，先基于新 `index.html`，不要再把客户默认入口放回旧施工稿。

## 2026-07-09 HTML 甲方可看模式救援

### 输入

- 夏夏指出：今天走过的后台需求如果连一份能讲清楚事、能给甲方看的单页 HTML 都拿不出来，就等于劳动成果没有被接住。
- 当前问题不是缺字段，而是客户界面、施工批注、字段备忘混在同一视觉层，页面显脏、显烂尾。

### 动作

- `换届选举系统-UI优化.html` 默认切为“甲方模式”：
  - 隐藏 `anchor-note`、`field-map`、`anchor-mini`、系统施工导览和母版施工表。
  - 新增 `开发锚点` 按钮，点击后给开发/后续施工查看完整锚点。
- 首页新增“子管理后台首页 / 涧口社区第十五届换届选举工作台”交付总览：
  - 讲清登录绑定、归属地、母活动、阶段表、岗位、材料候选人链路。
  - 默认示例聚焦 `经办 / 涧口社区 / 15000000000`。
- 修口径：
  - 登录角色只保留真实后端枚举：`超级管理/经办/运营/审核`。
  - `notice-v2/list?electionId&stageKey` 改为已支持。
  - 候选结果不再写成已同步 `positions.elected_candidates`。
  - “我是选民/选民登记”单独锚到 `election_voters`，标 `API TODO`。
- 修线下 HTML 交互风险：
  - `descOverlay/dlOverlay` 点击绑定移入 `DOMContentLoaded`，避免脚本在弹层 DOM 出现前报错。

### 验证

- HTML 内 `<script>` 用 `new Function` 语法检查通过。
- 关键锚点/默认入口可搜索：`delivery-subadmin-loop-001`、`election-voter-register-001`、`经办（子管理视角）`、`15000000000`、`涧口社区`、`GET /notice-v2/list?electionId&stageKey`。
- 脏词检查无业务外辱骂文本；搜索命中只剩“岗位”等正常词。
- `git diff --check -- 换届选举系统-UI优化.html` 仅提示 LF/CRLF。
- 未做 Playwright 截图验证：项目根和 `admin` 下未发现本地 `playwright` 包，本轮不联网安装。

### 接力棒

- 这份 HTML 现在同一文件有两种模式：默认给甲方看；点“开发锚点”给后续施工看。
- 下一刀若继续后台真实页面，应按这份 HTML 的默认甲方模式改真实 Vue 页面，不要把开发批注塞进业务 UI。

## 2026-07-09 干净后台 HTML 活动列表纠偏

### 输入

- 夏夏纠偏：“列表是什么知道吗”，要求定位对话相关区位和 sub 摘要，不要再把仪表盘做成侧边栏或说明稿。
- 小 mini 只读核查确认：`deliverables/client-admin-html/index.html` 的“最近 5 场活动”仍是 5 个届次标签 + 单个“进入当前母版”按钮，不是真活动记录列表。

### 动作

- `deliverables/client-admin-html/index.html` 新增 `state.activities`，保留最近 5 场活动的 `id/session/name/method/date/status/stage/archiveCount`。
- 把“最近 5 场活动”改为 `#recentActivityList` 渲染的 5 条活动记录，每条是独立可点击行。
- 新增 `selectedActivityId/getCurrentActivity/selectActivity(id)`：点击活动记录后保存当前活动，顶部标题、路径中的归属地/活动名、当前活动指标同步切换，并进入母版详情页。
- 清掉首屏“这是仪表盘的快捷凝练”说明稿口吻，改为正常后台文案。
- 调整三块速览网格 `align-items:start`，避免活动列表把前两块撑成空白大卡。

### 验证

- HTML 内脚本语法检查通过。
- `git diff --check -- deliverables/client-admin-html/index.html` 通过。
- Chrome headless 截图生成 `deliverables/client-admin-html/screenshot-dashboard.png`：确认首页右侧为 5 条活动记录列表，左/中速览卡不再被撑高。
- 关键词核查：`recentActivityList`、`activity-row`、`selectActivity`、`最近 5 场活动列表` 均存在；旧标签式届次行已消失。

### 接力棒

- 这次“列表”的定义已压实：最近 5 场活动必须是活动记录列表，每条可点击进入对应母版，不允许再退回标签/单按钮。
- 下一刀继续清理交付版里“验收说明 / 演示 / 正式施工锚点 / TODO”这些施工口吻，但不要因此删除旧 HTML 或旧锚点稿。

## 2026-07-09 母版详情页承接纠偏

### 输入

- 夏夏指出：HTML 都整理不清楚，因为仪表盘下去就是详情页面；一个都做不清楚，就应该老老实实搜索关键词，找到原文，理解清晰需求。
- 本轮关键词回捞：`仪表盘/面板/活动列表/母版/下钻/详情/岗位/花名册`。
- 小 mini 只读打捞确认真实链路：仪表盘是入口和速览，活动列表是下钻门，母版详情页才是主战场。

### 原文依据

- `#83 / 3550`：登录即筛选归属地，进入村子面板；面板是各个板块的调用，没有新增业务真相。
- `#175 / 3642`：全部活动筛选到某活动，点击查看详情，下钻到编辑选举活动母版。
- `#183 / 3650`：确定某村某活动后，下钻进详情页；详情页里有阶段公告、归档材料、岗位、候选人、公告。
- 项目离线文档 5.2/5.3：本村/社区活动列表点击进入 `/election/:id`；母活动详情页解释阶段、公告、材料、归档。

### 动作

- `deliverables/client-admin-html/index.html` 的母版详情从薄 4 行阶段表改为母表主体。
- 阶段数据扩为 11 阶段，沿用项目已有的村/社区换届阶段口径。
- 固定表头改为：`#/阶段名称/开始日期/结束日期/天数/核心工作/关联材料/上传文件/公告编辑`。
- 增加母版摘要：活动 ID、选举方式、阶段数、公告数、上传数。
- 增加同场承接卡：岗位、材料、候选人、归档，作为从详情页继续进入各模块的入口。
- 增加 `uploadStage` 作为阶段上传演示动作；公告编辑仍先复用阶段公告计数入口。
- 增加 `#mother` 深链，方便直接打开母版详情验证。
- localStorage key 从 `v1` 升到 `v2`，避免浏览器继续使用旧 4 阶段缓存。

### 验证

- HTML 内脚本语法检查通过。
- `git diff --check -- deliverables/client-admin-html/index.html` 通过。
- Chrome headless 截图生成 `deliverables/client-admin-html/screenshot-mother-detail.png`，确认母版详情页显示 11 阶段、固定表头、上传文件、公告编辑和活动摘要。

### 接力棒

- 这次纠偏的主结论：仪表盘不能承担详情页职责；仪表盘只做入口和速览，`/election/:id` 母版详情必须承接阶段、公告、材料、岗位、候选人、归档。
- 下一刀如果继续完善 HTML，优先把“公告编辑”做成阶段公告列表弹窗 + `+新增`，再把“上传文件”接到归档文件夹语义；不要回头在仪表盘堆细节。

## 2026-07-09 仪表盘产品形态重做

### 输入

- 夏夏给出对照截图，指出同样只给 4 个文档和历史对话，别人能生成真正后台形态；当前 HTML 仍像需求块堆叠，不像可交付后台。
- 要求收下这个差距，不再用“结构补了”代替产品形态。

### 动作

- 重做 `deliverables/client-admin-html/index.html` 首页布局：
  - `.app.is-open` 改为左侧导航 + 主内容双栏后台布局。
  - 左侧导航常驻，增加“办理中 / 收尾归档”业务分组。
  - 隐藏顶部横向胶囊导航，避免和侧边栏重复。
  - 顶部统计改为 4 张状态卡：岗位、候选人、待审材料、公告。
  - 主区改为“选举时间线 · 11 阶段 / 45 天”，包含 11 个圆点进度和阶段卡片。
  - 右侧改为“最近选举活动”和“工作提示”。
  - 顶部“验收说明”按钮移出首屏，替换为业务动作“刷新”。
- 补 `renderDashboard/renderTimelineDots/renderTimelineCards`，由 `state.stages/activities/materials/candidates` 渲染首页。
- 阶段日期补齐为 45 天，并把 localStorage key 升到 `v3`，避免旧 4 阶段或 19 天缓存污染截图。

### 验证

- HTML 内脚本语法检查通过。
- `git diff --check -- deliverables/client-admin-html/index.html` 通过。
- Chrome headless 截图生成 `deliverables/client-admin-html/screenshot-dashboard.png`，确认首屏为后台工作台形态：左侧业务导航、顶部状态卡、主区 11 阶段时间线、右侧最近活动和工作提示。

### 接力棒

- 首页质量线已抬高：不是说明稿，不是模块堆叠，不是横向胶囊导航复刻；子管理进入后第一眼必须是可操作后台工作台。
- 下一刀仍按上条：在母版详情里补“阶段公告弹窗列表 + 新增”和“上传文件归档语义”，不要回首页堆更多卡片。
